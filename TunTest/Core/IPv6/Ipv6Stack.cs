...
public class Ipv6Stack : IPacketProcessor
{
    ...
    public void ProcessPacket(ReadOnlySpan<byte> packetData)
    {
        ...
        switch (ipv6Header.NextHeader)
        {
            ...
            default:
                ...
                Console.WriteLine(
                    $"[Stack] Unbekanntes Protokoll {ipv6Header.NextHeader}. Sende ICMPv6 Unreachable...");
    ...
    private void HandleUdpEcho(Ipv6HeaderView requestIpv6, UdpHeaderView udpHeader)
    {
        ...
        var newUdpLength = (ushort)(8 + udpHeader.Payload.Length);
        
        // IPv6-Header
        var ipv6Writer = new Ipv6Writer(replyBuffer);
        var udpAndPayloadSpan = ipv6Writer.WriteHeader(
            sourceIp: requestIpv6.DestinationAddressBytes,
            destIp: requestIpv6.SourceAddressBytes,
            // TODO: Magische Protokollnummer 17 durch eine zentrale IpProtocol.Udp-Konstante ersetzen.
            nextHeader: 17 // UDP
        );

        // UDP-Header
        var udpWriter = new UdpWriter(udpAndPayloadSpan, requestIpv6.DestinationAddressBytes, requestIpv6.SourceAddressBytes);
        var payloadSpan = udpWriter.WriteHeader(udpHeader.DestinationPort, udpHeader.SourcePort);
        
        // Payload einfügen
        udpHeader.Payload.CopyTo(payloadSpan);

        udpWriter.FinalizePacket((ushort)udpHeader.Payload.Length);
        ipv6Writer.FinalizePacket(newUdpLength);
    ...
    private void HandleDaytime(Ipv6HeaderView requestIpv6, UdpHeaderView udpHeader)
    {
        ...
        // Payload einfügen
        Encoding.UTF8.GetBytes(timeString, payloadSpan);
        
        udpWriter.FinalizePacket((ushort)udpPayloadLength );
        ipv6Writer.FinalizePacket(newUdpLength);
        _packetSender.SendPacket(replyBuffer);
    }
